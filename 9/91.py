...
def move_tail (h, t):
    ...
    if (h[0] == t[0] and h[1] == t[1]):
    ...
    elif abs(h[0]-t[0]) == 2 and abs(h[1]-t[1]) == 1:   # diagonal, col 2 apart 
        if t[0] < h[0]:
            t[0] += 1
        else:
            t[0] -= 1
    ...
    elif abs(h[1]-t[1]) == 2 and abs(h[0]-t[0]) == 1:   # diagonal, row 2 apart 
        if t[1] < h[1]:
            t[1] += 1
        else:
            t[1] -= 1
        ...
        raise RuntimeError ("move_tail: Invalid Positions - h=%s, t=%s" % (h,t))
